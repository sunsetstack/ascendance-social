...
describe("correlationIdMiddleware", () => {
	...
	it("reuses an incoming x-request-id header", (done) => {
		...
		correlationIdMiddleware(req, res, () => {
			...
			expect(res.setHeader.calledOnceWithExactly("X-Request-ID", "request-abc")).to.be.true;
	...
	it("creates a new correlation id when the request has none", (done) => {
		...
		correlationIdMiddleware(req, res, () => {
			...
			expect(res.setHeader.calledOnceWithExactly("X-Request-ID", req.correlationId)).to.be.true;
