...
export class UserPreferenceRepository extends BaseRepository<IUserPreference> {
  ...
  async getTopUserTags(userId: string, limit = 5): Promise<IUserPreference[]> {
    try {
      ...
      logger.error("[UserPreferenceRepository] Failed to get top user tags", {
        userId,
        limit,
        error,
      });
  ...
  async getUsersWithTagPreferences(
    tags: string[],
    minScore: number = 1,
  ): Promise<IUser[]> {
    try {
  ...
  async deleteManyByUserId(userId: string): Promise<number> {
    try {
      ...
      logger.error(
        "[UserPreferenceRepository] Failed to delete user preferences",
        { userId, error },
      );
      if (error instanceof Error) {
        throw wrapError(error);
